...
class Database:
    """Database layer for storing job application data using SQLite."""
    ...
    def save_search_history(self, search_history: SearchHistory) -> bool:
        """
        Save a search history entry to the database.
        
        Args:
            search_history: SearchHistory object to save
        ...
        try:
            ...
            # Convert search history object to dict for storage
            search_dict = search_history.to_dict()
            
            # Convert filters to JSON string
            search_dict["filters"] = json.dumps(search_dict.get("filters", {}))
            
            # Insert new search history (search history entries are immutable)
            cursor.execute('''
            INSERT INTO search_history (id, keywords, location, filters,
                                      date_searched, job_count)
            VALUES (?, ?, ?, ?, ?, ?)
            ''', (
                search_dict["id"], search_dict["keywords"], search_dict["location"],
                search_dict["filters"], search_dict["date_searched"], search_dict["job_count"]
            ))
            
            conn.commit()
            logger.info(f"Saved search history: {search_dict['id']}")
            return True
        except Exception as e:
    ...
    def get_cached_search_results(self, keywords: str, location: str, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            search_id = hashlib.md5(f"{keywords}|{location}|{str(filters)}".encode()).hexdigest()
            ...
            # First try to get job_ids from search_job_mapping table
            try:
                cursor.execute('''
                ...
                # Try getting job_ids from search_history if they're stored there
                try:
                    cursor.execute('''
                    ...
                    logger.debug(f"Could not get job_ids from search_history: {e}")
        ...
        try:
            ...
            if existing:
                ...
                ''', (
                    ...
                    date_searched, len(job_ids), search_id
                ))
                logger.info(f"Updated search history: {search_id}")
            else:
                # Insert new search history
                try:
                    # First try with job_ids column (if it exists)
                    ...
                    # If job_ids column doesn't exist, use the original schema
                    cursor.execute('''
                    INSERT INTO search_history (id, keywords, location, filters,
                                              date_searched, job_count)
                    VALUES (?, ?, ?, ?, ?, ?)
