...
class DBCacheManager:
    ...
    ensuring data consistency and optimizing performance.
    """
    ...
    def save_search_results(self, keywords: str, location: str, filters: Dict[str, Any], 
        ...
        if self.db:
            ...
            success = self.db.save_search_history(keywords, location, filters, job_ids, search_id)
        ...
        if self.job_cache:
            for job_dict in job_listings:
                ...
                try:
                    ...
                    import logging
                    logging.getLogger(__name__).error(f"Error adding job to cache: {e}")
                    success = False
