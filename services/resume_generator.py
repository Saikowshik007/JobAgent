...
class ResumeGenerator:
    ...
    async def generate_resume(self, job_id: str, template: str = "standard",
        ...
        job_dict = await self.cache_manager.get_job(job_id, self.user.id)
        if not job_dict:
            raise ValueError(f"Job not found with ID: {job_id} for user: {self.user.id}")

        # Check for existing resumes linked to this job
        existing_resumes = await self.cache_manager.get_resumes_for_job(job_id, self.user.id)
        ...
        await self.cache_manager.set_resume_status(
            resume_id, self.user.id, ResumeGenerationStatus.PENDING
        )

        # Start background generation (non-blocking)
        asyncio.create_task(self._generate_resume_background(
            job_dict, resume_id, template, customize, resume_data, existing_resumes, handle_existing, include_objective
        ))
    ...
    async def _generate_resume_background(self, job_dict: dict, resume_id: str, template: str,
        ...
        try:
            logger.info(f"Starting resume generation for job {job_dict.get('id')} for user {self.user.id}")

            # Update status to in progress
            await self.cache_manager.set_resume_status(
                resume_id, self.user.id, ResumeGenerationStatus.IN_PROGRESS
            )
            ...
            yaml_content = await loop.run_in_executor(
                ...
                job_dict, resume_data, include_objective  # Pass include_objective here
            )
            ...
            await self.cache_manager.save_resume(resume, self.user.id)
            ...
            if existing_resumes:
                if handle_existing == "replace":
                    ...
                    for old_resume in existing_resumes:
                        try:
                            success = await self.cache_manager.delete_resume(old_resume.id, self.user.id)
                            if success:
                                logger.info(f"Deleted old resume {old_resume.id} for job {job_dict.get('id')}")
                                # Remove from generation cache too
                                await self.cache_manager.remove_resume_status(old_resume.id, self.user.id)
                            else:
            ...
            await self.cache_manager.set_resume_status(
                resume_id, self.user.id, ResumeGenerationStatus.COMPLETED,
            ...
            await self.cache_manager.update_job_status(
                job_dict.get('id'), self.user.id, JobStatus.RESUME_GENERATED
            )

            logger.info(f"Resume generation completed for job {job_dict.get('id')} for user {self.user.id}")

        except Exception as e:
            logger.error(f"Error generating resume for job {job_dict.get('id')} for user {self.user.id}: {e}")

            # Update cache with failed status
            await self.cache_manager.set_resume_status(
                resume_id, self.user.id, ResumeGenerationStatus.FAILED,
                error=str(e)
            )
    async def _update_job_with_resume_id(self, job_id: str, resume_id: str):
        ...
        try:
            ...
            success = await self.cache_manager.update_job_resume_id(job_id, self.user.id, resume_id)
    ...
    def _generate_resume_sync(self, job_dict: dict, resume_data: Optional[Dict[str, Any]], include_objective: bool = True) -> str:
    ...
    async def check_resume_status(self, resume_id: str) -> Dict[str, Any]:
        ...
        # First check cache
        cache_entry = await self.cache_manager.get_resume_status(resume_id, self.user.id)
        ...
        if cache_entry:
            ...
            if status == ResumeGenerationStatus.COMPLETED.value:
                ...
                try:
                    resume = await self.cache_manager.get_resume(resume_id, self.user.id)
                    if resume and resume.job_id:
                        job_dict = await self.cache_manager.get_job(resume.job_id, self.user.id)
                        if job_dict:
        ...
        resume = await self.cache_manager.get_resume(resume_id, self.user.id)
        if not resume:
            raise ValueError(f"Resume not found with ID: {resume_id} for user: {self.user.id}")
    ...
    async def upload_resume(self, file_path: str, file_content: bytes, job_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            success = await self.cache_manager.save_resume(resume, self.user.id)
            ...
            logger.error(f"Error uploading resume for user {self.user.id}: {e}")
            raise
    ...
    async def replace_job_resume(self, job_id: str, new_resume_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            new_resume = await self.cache_manager.get_resume(new_resume_id, self.user.id)
            if not new_resume:
                raise ValueError(f"Resume not found with ID: {new_resume_id} for user: {self.user.id}")

            # Get current resume(s) for this job
            current_resumes = await self.cache_manager.get_resumes_for_job(job_id, self.user.id)

            # Update the job to point to the new resume
            success = await self.cache_manager.update_job_resume_id(job_id, self.user.id, new_resume_id)
            ...
            if new_resume.job_id != job_id:
                ...
                await self.cache_manager.save_resume(new_resume, self.user.id)
            ...
            for old_resume in current_resumes:
                if old_resume.id != new_resume_id:  # Don't orphan the new resume we just linked
                    ...
                    await self.cache_manager.save_resume(old_resume, self.user.id)
            ...
            logger.error(f"Error replacing job resume for job {job_id} for user {self.user.id}: {e}")
            raise
